...
class MenuControl
{
    ...
    private function ModificarRelacion($param)
    {
        ...
        $objMenuRolantes = $MenuRolControl->buscar(['idmenu' => $param['idmenu']]);
        $dataMR = [
            'idmenu' => $objMenuRolantes[0]->getObjMenu()->getID(),
            'idrol' => $objMenuRolantes[0]->getObjRol()->getID()
        ];
        $MenuRolControl->baja($dataMR);
        $resp = $MenuRolControl->alta(['idmenu' => $param['idmenu'], 'idrol' => $param['idrol']]);
        return $resp;
    }
    ...
    public function darAlta($param)
    {
        ...
        $menuRolControl = new MenuRolControl();
        $listaRol = $rolControl->buscar($param);
        $listaMenu = $menuControl->buscar($param);


        $respuesta = $menuRolControl->alta(['idrol' => $listaRol[0]->getID(), 'idmenu' => $listaMenu[0]->getID()]);
        if (!$respuesta) {
    ...
    public function listarMenuesTabla()
    {
        ...
        if (count($list) > 0) {
            ...
            foreach ($list as $menuActual) {
                ...
                $mrActual = $menuRolControl->buscar(['idmenu'=>$menuActual->getID()]);
                if(!empty($mrActual)){
                    $rol = "";
                ...
                $hijos = $menuControl->buscar(['idpadre'=>$menuActual->getID()]);                
